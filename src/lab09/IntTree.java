...
public class IntTree {
	...
	private String getLevel(IntTreeNode root, int level, int target){
		if(root == null) {
			return "";
		}
		...
		return getLevel(root.left, level + 1, target) + getLevel(root.right, level + 1, target);
	...
	public void makePerfect(){
		...
		int height = countHeight(overallRoot);
		makePerfect(overallRoot, height);
	}
	public int countHeight(IntTreeNode root) {
		if(root == null){
		  return 0;
		} else {
			int v1 = root.left != null ? countHeight(root.left) : -1;
			int v2 = root.right != null ? countHeight(root.right) : -1;
			return Math.max( v1, v2) + 1;
		}
	}
	private void makePerfect(IntTreeNode root, int height) {
		if (height == 0) {
			return;
		}
		if(root.left == null) {
		...
		makePerfect(root.left, height-1);
		makePerfect(root.right, height-1);
	}
}